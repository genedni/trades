...
def log_package_versions():
    ...
    for pkg in packages:
        try:
            ...
            logger.info(f"Package {pkg} version: %s", version)
...
def create_stock_chart():
    try:
        ...
        if missing_columns:
            ...
            raise ValueError(f"Missing or empty candlestick columns: %s", missing_columns)
        ...
        logger.debug("Index timezone: %s", df.index.tz)
        end_date = pd.Timestamp(df.index[-1]).tz_localize(None)  # Remove timezone
        start_date = end_date - timedelta(days=90)
        logger.debug("Setting initial viewport from %s to %s", start_date, end_date)

        # Convert index to timezone-naive for slicing
        ...
        # Serialize index as ISO date strings (YYYY-MM-DD)
    ...
    def update_y_axis(relayout_data, stock_data):
        try:
            ...
            if start is not None and end is not None and pd.notna(start) and pd.notna(end):
                ...
                logger.debug("data_start: {data_start} data_end: {data_end}")
